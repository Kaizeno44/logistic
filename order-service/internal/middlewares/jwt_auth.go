...
func JWTAuth() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if claims, ok := token.Claims.(jwt.MapClaims); ok && token.Valid {
			...
			c.Set("role", claims["role"]) // Lưu thêm role vào context
		}

		c.Next() // Cho phép đi tiếp vào API
	}
}

// 3. Middleware bảo vệ API dành riêng cho Admin
func AdminAuth() gin.HandlerFunc {
	return func(c *gin.Context) {
		role, exists := c.Get("role")
		if !exists || role != "admin" {
			c.AbortWithStatusJSON(http.StatusForbidden, gin.H{"error": "Bạn không có quyền truy cập (Yêu cầu Admin)!"})
			return
		}
		c.Next()
	}
